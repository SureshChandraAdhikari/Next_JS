...
export default function SignUpPage() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen py-2">
            ...
            <hr />
            <label className="text-xl text-center text-green-400 m-1" htmlFor="username">UserName</label>
            <input className="p-4 border border-gray-600" id="username" type="text" placeholder="Enter Your UserName" value={user.username} onChange={(e) => setUser({ ...user, username: e.target.value })} />
            ...
            <button className="m-2 relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-500 to-blue-500 group-hover:from-cyan-500 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-cyan-200 dark:focus:ring-cyan-800"onClick={onSignUp} >
<span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
SignUp Here
</span>

</button><Link href="/login">Visit Login Page</Link>
        </div>
    )
}